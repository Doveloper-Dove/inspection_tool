...
- 프로그램 자체의 실행당 AI 요청 횟수 제한은 없습니다.
- 서비스별 요금, 할당량, 분당 요청 제한은 각 AI 제공업체 정책을 따릅니다.
- 댓글 중복 검사는 하지 않습니다.
...
# =============================================================================
# ★★★ 중요 설정 2: 프로그램 자체 API 호출 횟수 제한 해제 ★★★
# =============================================================================
# 실행당 5회 제한을 포함해 프로그램 내부에서 걸었던 API 호출 상한을 제거했습니다.
# 처리할 기사 묶음이 남아 있으면 사용자가 중지하거나 서비스 오류가 발생할 때까지
# 계속 분석합니다.
#
# 주의:
# - 각 서비스의 결제 한도, 분당 요청 수, 일일 할당량, 토큰 한도는 그대로 적용됩니다.
# - 요청 실패 시의 최대 재시도 횟수는 오류 무한 반복을 막기 위한 안전장치입니다.
LOCAL_API_CALL_LIMIT = None
# =============================================================================
# ★★★ 로컬 API 호출 제한 없음 ★★★
# =============================================================================
...
PROVIDERS: dict[str, ProviderInfo] = {
    ...
    "Vertex AI": ProviderInfo(
        code="vertex_ai",
        display_name="Google Vertex AI (Express Mode)",
        default_model="gemini-3.1-flash-lite",
        endpoint="",
        key_required=True,
        key_hint="Google Cloud Vertex AI Express Mode에서 발급한 API 키",
    ),
    "Groq": ProviderInfo(
        code="groq",
        display_name="Groq",
    ...
    "Ollama": ProviderInfo(
        ...
        key_required=False,
        key_hint="API 키는 필요 없지만 DeepSeek V4 Flash Cloud 사용을 위해 Ollama 로그인이 필요할 수 있습니다.",
    ),
}
...
@dataclass
class CallCounter:
    """실제 AI 네트워크 요청 횟수만 기록하며 상한은 두지 않습니다."""
...
def request_vertex_ai(
    config: RunConfig,
    prompt: str,
) -> str:
    """
    Google Vertex AI Express Mode를 API 키 하나로 호출합니다.

    배포용 GUI의 인증 방식을 통일하기 위해 표준 Vertex AI의 서비스 계정 JSON 대신
    Express Mode API 키 방식을 사용합니다. API 키는 GUI 메모리에서만 사용됩니다.
    """
    try:
        ...
        raise RuntimeError(
            "Vertex AI 사용에 필요한 google-genai가 설치되어 있지 않습니다.\n"
    ...
    try:
        ...
        raise RuntimeError(f"Vertex AI 요청 실패: {error}") from error

    content = safe_text(getattr(response, "text", "")).strip()
    if not content:
        raise RuntimeError("Vertex AI가 빈 응답을 반환했습니다.")
    return content
...
def request_provider(
    ...
    if config.provider.code == "vertex_ai":
        return request_vertex_ai(config, prompt)
...
def call_batch(
    ...
    for attempt in range(1, config.max_retries + 1):
        ...
        # =====================================================================
        # ★★★ 실제 네트워크 요청 횟수 기록 — 상한 없음 ★★★
        # =====================================================================
        # 성공·실패·재시도를 모두 기록하지만 프로그램 내부에서 차단하지 않습니다.
        call_number = counter.consume()
        # =====================================================================
        # ★★★ AI 요청 횟수 기록 구간 끝 ★★★
        # =====================================================================
        ...
        emit(
            "log",
            text=f"실제 AI 요청 누적 {call_number}회",
        )
...
def run_analysis(
    ...
    try:
        ...
        for batch_position, indices in enumerate(
            ...
            try:
                result_by_id, call_number = call_batch(
                    config,
                    batch_rows,
                    counter,
                    stop_event,
...
class DiscourseClassifierGUI(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            container,
            text=(
                "기사별 모든 댓글을 요청 1회에 묶으며, 프로그램 자체 호출 횟수 제한은 없습니다. "
                "유료 API에서는 비용과 서비스별 할당량을 반드시 확인하십시오."
            ),
    ...
    def _start(self) -> None:
        ...
        self.calls_var.set("AI 요청 사용: 0회")
        ...
        try:
            while True:
                ...
                if event_type == "log":
                    ...
                    self.progress.configure(value=payload["value"])
