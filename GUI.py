"""
네이버 뉴스 제목/본문 → Word2Vec 입력 TXT 생성기 v16_strict_dedup
...
def extract_article_links_from_search_soup(
    ...
    v16_strict_dedup 수정점:
    - 같은 검색 결과 카드에서는 네이버뉴스 링크를 우선하고, 원문 링크는 네이버뉴스 링크가 없을 때만 사용합니다.
    - URL이 달라도 제목이 같으면 같은 기사로 보고 링크 수집 단계에서 제거합니다.
    - 검색 결과 카드에서 링크가 잡혔으면 전체 a[href] fallback을 실행하지 않습니다.
      이 fallback이 원문 링크/네이버 링크 중복과 네이버 서비스 잡링크를 크게 늘리는 원인이었기 때문입니다.
    ...
    def add_link(url: str | None, search_title: str = "") -> None:
        ...
        # 제목이 같으면 원문/네이버 링크 URL이 달라도 같은 기사로 봅니다.
...
def collect_article_links_by_date_chunks(
    ...
    all_links: list[str] = []
    seen: set[str] = set()
    seen_search_title_keys: set[str] = set()
    ...
    try:
        for ds, de in date_ranges:
            ...
            for link in chunk_links:
                link_title_key = SEARCH_TITLE_KEY_BY_URL.get(link, "")

                # 날짜 조각이 달라도 검색 결과 제목이 같으면 같은 기사로 봅니다.
                # 원문 링크와 네이버 링크가 URL만 다르게 반복 수집되는 문제를 여기서 한 번 더 막습니다.
                if link_title_key and link_title_key in seen_search_title_keys:
                    search_title_duplicate_count += 1
                    continue
                ...
                if link not in seen:
                    ...
                    if link_title_key:
                        seen_search_title_keys.add(link_title_key)
...
def make_word2vec_line(title: str, body: str, body_chars: int = DEFAULT_BODY_CHARS) -> str:
    """
    기사 제목과 본문을 Word2Vec 입력용 한 줄 문장으로 합칩니다.
...
def save_word2vec_txt(articles: list[dict[str, str]], output_txt: str, body_chars: int) -> int:
...
def save_debug_csv(articles: list[dict[str, str]], output_csv: str) -> None:
    """확인용 CSV를 저장합니다."""
    fieldnames = ["url", "source_type", "title", "body"]
...
def crawl_and_make_word2vec_input(
    ...
    for url in tqdm(links, desc="제목/본문 수집"):
    ...
    print(f"중복 기사로 제외된 총수: {duplicate_title_count + duplicate_content_count}")
...
class NaverNewsCrawlerGUI(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        option_frame.grid(row=6, column=0, columnspan=4, sticky="ew", pady=(8, 0))
    ...
    def _run_worker(self, params: dict) -> None:
        ...
        try:
            with contextlib.redirect_stdout(writer), contextlib.redirect_stderr(writer):
                print("=" * 60)
                print("네이버 뉴스 제목/본문 → Word2Vec 입력 TXT 생성기 GUI v16_strict_dedup")
                print("=" * 60)
